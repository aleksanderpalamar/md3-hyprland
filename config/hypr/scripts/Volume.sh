...
notify_user() {
    if [[ "$(get_volume)" == "Muted" ]]; then
        ...
        notify-send -e -h int:value:"$(get_volume | sed 's/%//')" -h string:x-canonical-private-synchronous:volume_notif -h boolean:SWAYNC_BYPASS_DND:true -u low -i "$(get_icon)" " $(i18n MSG_VOLUME_LEVEL):" " $(get_volume)"
    fi
}
